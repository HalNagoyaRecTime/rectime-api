...
auth.post('/logout', async c => {
  ...
  if (clientType === 'mobile') {
    ...
    // インデックスから現在のリフレッシュトークンIDを取得して両エントリを削除する
    // クライアント提供の値を使わないことで所有権チェック漏れと残存トークンを防ぐ
    const currentRefreshTokenId = await c.env.AUTH_KV.get(
      `mobile_refresh_by_user:${claims.sub}`
    );
    if (currentRefreshTokenId) {
      await c.env.AUTH_KV.delete(`mobile_refresh:${currentRefreshTokenId}`);
    }
...
auth.post('/refresh', async c => {
  ...
  // 旧エントリを先に削除して二重使用ウィンドウをなくす
  // 失敗しても再ログインを要求するだけで済む安全な失敗モード
